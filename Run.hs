...
main :: IO ()
main = driver $ do
  putBufLogStrLn "Sigma16 M1 system starting"
  lift $ writeFile writeFileName ""
  lift $ writeFile logFileName ""  -- ensure it's empty

--  objectCode <- liftIO getObject
  objectCode <- getObject
...
getObject :: StateT (SysState b) IO [Int]
getObject = do
  args <- lift getArgs
--  putStrLn (show args)
  case args of
    [] -> do
      lift $ putStrLn "Usage: :main path/to/objectfile"
      return []
    (a:_) -> do
      prefixFile <- lift $ maybeRead "fileprefix.txt"
--      putStrLn ("file prefix = " ++ show prefixFile)
      ...
      code <- lift $ readObjectCode fullPath
      putBufLogStrLn ("Object code is " ++ show code)
      return code
...
--    liftIO $ putLogStrLn (" *** Peek at register R" ++ show regnum)
...
--  liftIO $ putLogStrLn ("Register value = " ++ show bs)
--  liftIO $ putLogStrLn ("***** Peek R" ++ show regnum ++ " = " ++ bitsHex 4 bs)
  putBufLogStrLn ("*** Cycle " ++ show i
                   ++ ": I/O fetch R"
                   ++ show regnum ++ " = " ++ bitsHex 4 bs)
--  runFormat True displayFullSignals
--  lift $ putLogStrLn "\npeekReg about to runFormat"
  runFormat peekmode
--  lift $ putLogStrLn "\npeekReg back from runFormat"
...
--    True -> runFormat True
  ...
  advancePeeks
  advanceFlagTable
--  lift $ putLogStrLn "\npeekReg about to advanceFormat"
  advanceFormat
--  lift $ putLogStrLn "\npeekReg back from advanceFormat"
  incrementCycleCount
  return bs
...
cmdDmaSigs :: String -> StateT (SysState b) IO ()
cmdDmaSigs flag = do
  if flag == "on"
    then do s <- get
...
readMemBlock :: Int -> Int -> [Int] -> StateT (SysState DriverState) IO [Int]
...
pokeMem a x = do
  ...
  conditional displayFullSignals $ do
    putBufLogStrLn (take 80 (repeat '-'))
  putBufLogStr ("Cycle " ++ show i ++ ".  ")
  putBufLogStrLn $ "pokeMem a = " ++ bitsHex 4 abs ++ " value = " ++ bitsHex 4 xbs
-- reset = 0
-- io_DMA = 1
-- io_memStore = 1
-- io_memFetch = 0
...
readFromUser n buf = do
  putLogStrLn "\n*****"
  putLogStrLn ("Read request, buffer size = " ++ show n ++ " characters")
  putLogStrLn ("Input will be padded with spaces, and extra characters ignored")
  xs <- hGetLine stdin
  let ys = take n (xs ++ repeat ' ')
  putLogStrLn ("Input data = <" ++ ys ++ ">")
  putLogStrLn "\n*****"
  return ys
--  putLogStrLn ("Read request, buffer size = " ++ show n)
--  if length buf > 0
--    then putLogStrLn ("Data you enter will follow buffered data: <" ++ buf ++ ">")
--    else return ()                  
...
peekMem addr = do
  ...
  conditional displayFullSignals $ do
    putBufLogStrLn (take 80 (repeat '-'))
    putBufLogStr ("Cycle " ++ show i ++ ".  ")
--  liftIO $ putLogStrLn (" *** Peek at memory address " ++ show addr)
...
--    printOutPorts
  ...
  let bs = map current a
  putBufLogStrLn ("*** Cycle " ++ show i
--                  ++ ": I/O fetch Mem[" ++ show addr ++ "] = " ++ bitsHex 4 bs)
...
printHelp = do
  ...
  printLine "  logfile on"
  printLine "  logfile off"
...
m1ClockCycle = do
--  liftIO $ putLogStrLn ("m1ClockCycle starting")
  s <- get
  let i = cycleCount s
  let inp = inPortList s
  let outp = outPortList s
